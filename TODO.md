...
*   **`P0 — Core Credibility & Intelligence Baseline`** (Section 1 - 5: Test Debt + FFF + Tree-sitter + Context Registry & Judge)
*   **`P1 — Retrieval & Document Fidelity`** (Section 9 - 11: Hybrid Search + Disk-backed Vector + kept-doc Universal IR)
*   **`P2 — Ecosystem Expansion`** (Section 6 - 8, 12: kept-vault Package Manager, Script Runtime, Notion Reconciler, TUI)

> ⚠️ **Gate Rule:** ห้ามเริ่มงานในหมวด **P2** จนกว่าทุกข้อในหมวด **P0** จะถูก Implement จริง, ผ่าน TDD Red-Green, และผ่าน `just check` 100% ครบถ้วน
...
- [x] ทำ TDD red สำหรับ unified MCP tool registration: `filesystem_find`, `filesystem_grep`, `filesystem_multi_grep`, `filesystem_rescan`, `filesystem_status`
- [x] สร้าง `FffManager` ใน `crate/kept-mcp/src/mcp/` เก็บ FFF instance ต่อ canonical root และ reuse index/watcher ตลอดอายุ MCP server
- [x] ให้ manager สร้าง instance เมื่อเรียก root ครั้งแรก, รอ readiness ด้วย timeout, reuse instance เดิม และ cleanup worker/watcher/cache เมื่อ server ปิด
- [x] ทำ `filesystem_find` คืน fuzzy path, size, modified time, binary state, Git status, score, pagination cursor และ total matches
- [x] ทำ `filesystem_grep` คืน path, line number, column, line content, context, match range และ cursor
- [x] ทำ `filesystem_multi_grep` ใช้ OR semantics ของ FFF พร้อม separate constraints
- [x] ทำ `filesystem_rescan` trigger rescan เฉพาะ root ที่ระบุ และคืน scan state ก่อน/หลังทำงาน
- [x] ทำ `filesystem_status` คืน active root, indexed file count, scanning state, watcher readiness, warmup state และ error ล่าสุด
- [x] validate canonical root ทุก tool และใช้ root guard ของ FFF สำหรับ filesystem root/home root
- [x] ทำ TDD red สำหรับ root isolation, binary file, unreadable metadata, query ไม่พบ, invalid constraint, stale cursor, initial scan timeout และ watcher overflow
- [x] ทำ MCP schema/versioning สำหรับ filesystem และ vault tools; ทุก tool มี input/output schema version ที่ตรวจได้
- [x] เพิ่ม dry-run และ permission boundary ให้ tools ที่จะเขียน vault/package state; filesystem search tools ยังคง read-only
- [x] ส่ง FFF/MCP logs ไป stderr เท่านั้นเพื่อไม่ทำลาย JSON-RPC stdio
- [x] รัน MCP stdio smoke ที่เรียก filesystem tools พร้อม document tools เดิมใน process `bl1nk-kept-mcp` เดียว

---

## 🟡 [P2] — Verification, Dogfooding & Search Engine (พิสูจน์กับคลังข้อมูลจริง)
...
## 🟢 [P3] — Document Fidelity & Inspection (ประมวลผลเอกสารความรู้)
...
## 🔵 [P4] — Ecosystem & Automation Expansion (ส่วนต่อขยายระบบ)
