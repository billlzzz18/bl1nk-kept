...
```rust
// Bad — redundant type annotation (compiler infers)
let results: Vec<String> = items.iter().map(|x| x.to_string()).collect();
...
// Better: delete comments, code is self-documenting
for item in items {
    process(item);
}
```
...
let filtered = items.into_iter().filter(|x| x.is_active()).collect::<Vec<_>>();
let count = filtered.len();
return count;

// Better
items.into_iter().filter(|x| x.is_active()).count()
```

**Suspicious (review first):**

```rust
// Suspicious — single-impl trait (might be over-abstraction)
trait FileScanner {
    fn scan(&self) -> Result<Vec<FileRecord>>;
}
impl FileScanner for FffScanner { ... }  // only impl

// Might be OK if: trait is used as type bound in generics
// Likely slop if: trait is never used as abstraction
```

```rust
// Suspicious — long match with many arms
match event {
    Event::Created(p) => { ... },
    Event::Modified(p) => { ... },
    Event::Deleted(p) => { ... },
    Event::Renamed(a, b) => { ... },
    Event::Accessed(p) => { ... },
    Event::MetadataChanged(p) => { ... },
    Event::Any => { ... },
}

// Might be OK if: exhaustive handling is required by design
// Likely slop if: most arms do the same thing
```

```rust
// Suspicious — defensive programming in internal code
...
// Might be OK if: this is a public API boundary
// Likely slop if: this is internal code with trusted input
```
