...
| ARCH-002 | **คำมั่นสัญญาด้านสถาปัตยกรรม (Full FFF & SQZ Adoption - ไม่ตัดทอนว่าแค่นี้พอ):** การสร้างระบบ Context Engine ต้องนำชิ้นส่วนและแรงบันดาลใจจาก `FFF` และ `SQZ` มาใช้ให้ครบวงจร (CLI สำหรับคน, MCP สำหรับ AI, C-ABI/Library/Interceptor สำหรับ Editor, และ Polyglot Tree-sitter Grammars); ห้าม Agent คิดเองเออเงว่าทำแค่ MCP อย่างเดียวแล้วพอ | `COMPLETE` | ADR 0004 กำหนด: reimplement SQZ patterns in-process ใน Rust เป็น explicit design choice (zero IPC latency, no external process dependency); `fff-search = "0.10.6"` dependency จริง; `judge.rs` implement SQZ-inspired admission decisions ครบวงจร (PASS/REFERENCE/DELTA/COMPRESS/DROP/WARN/BLOCK/RESOLVE); `scanner/fff.rs` เป็น FFF acquisition adapter; Editor integration (C-ABI + Tree-sitter) track ใน TODO 2.2.1 แยกต่างหาก | Editor integration ยังอยู่ใน TODO 2.2.1 — บังคับใช้ในทุกการตัดสินใจสถาปัตยกรรมต่อ |
| COGNITION-001 | ระบบ Zero-Regression Memory: `CorrectionLedger` (SQLite immutable, supersede history) ต้องผูกเข้ากับ `Judge` เป็น pre-acquisition guard — ก่อน observation เข้า context stream ต้อง block ถ้า content ตรงกับ `rejected_assertion` ที่บันทึกไว้ใน active correction | `COMPLETE` | `correction_ledger.rs`: `record`, `supersede`, `conflict_for`, `active_for`, `history_for`; `judge.rs`: `with_ledger`, `evaluate_with_ledger`; red tests: `test_judge_blocks_observation_conflicting_with_active_correction`, `test_judge_allows_observation_not_conflicting_with_ledger`; `just check` exit 0 (8/8 context_contract pass) | Reconcile step: อัปเดต `docs/specs/cognitive_guardrail_architecture.md` เมื่อสร้าง (P0.2 Reconcile open) |
| SEM-002 | **Semantic & Scope Disambiguation (P0.3):** `Judge.evaluate_intent()` คืน `Resolve` พร้อมช้อยส์สำหรับคำกำกวม (ทดสอบ/ปัญหา/ลบ/ทั้งหมด); `evaluate_scope()` Block implicit wider scope; `override_rate()` metric; Tier 1=Rust literal, Tier 2=host-agent Skill/Hook | `COMPLETE` | ADR 0004 (จัดทำแล้ว); `AdmissionDecision::Resolve` variant; `AMBIGUOUS_TERMS` static table; `IMPLICIT_WIDER_SCOPES` guard; `evaluate_intent`, `evaluate_scope`, `override_rate`, `record_override` ใน `judge.rs`; Red→Green: 7 test cases ใหม่ (15/15 context_contract ผ่าน); `context_admission_engine.md` อัปเดตแล้ว; TODO 2.3.3 ทุก checkbox ปิดแล้ว | Re-run `cargo test --test context_contract` ทุก session ที่แตะ judge.rs; ถ้าเพิ่มคำกำกวมใหม่ต้องเพิ่มใน `AMBIGUOUS_TERMS` และเขียน test case คู่กัน |
